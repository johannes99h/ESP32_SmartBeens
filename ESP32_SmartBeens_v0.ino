...
int sd_prepare_data_log(unsigned long long time, struct data& am2320_1, struct data& am2320_2, int co2_ppm, float co2_temperature, float weight, float batt_voltage) {
  ...
  unsigned int data_length = sprintf(data, "%d, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %d, %.1f, %.4f",
                                     time, am2320_1.temperature, am2320_1.humidity, am2320_2.temperature,
                                     am2320_2.humidity, -99.9, -99.9, co2_temperature,
                                     co2_ppm, weight, batt_voltage);
...
int sd_prepare_rtc_data_log(String timestamp, unsigned long long time, struct data& am2320_1, struct data& am2320_2, int co2_ppm, float co2_temperature, float weight, float batt_voltage) {
  char data[SD_WRITE_BUFFER];
...
int runtime_routine() {
  ...
  if (true == SD_CARD_INIT_SUCCESSFULL) {
    if (RTC_USED) {
      ...
      sd_prepare_rtc_data_log(timestamp, time_since_start, am2320_1_data, am2320_2_data, co2_ppm, co2_temperature, weight, batt_voltage);
    } else {
      ...
      sd_prepare_data_log(time_since_start, am2320_1_data, am2320_2_data, co2_ppm, co2_temperature, weight, batt_voltage);
    }
  }
  ...
  if (LTE_MODEM_USED) {
    ...
    if (0 == sim7070_network_check()) {
      ...
      sim7070_prepare_all_sensor_data_for_http_post(batt_voltage, weight, am2320_1_data, am2320_2_data);
      sim7070_pwr_down();
    }
  }
